...
from config.node_config import GENESIS_ACCOUNT, GENESIS_PRIV_KEY
...
class FileEciesUploadDownloadTest(ClientTestFramework):
    ...
    def run_test(self):
        # Use the signer's wallet private key as the recipient key for ECIES.
        # --encrypt derives the recipient pubkey from the signer's --key, so
        # the same private key must be used for --decrypt --private-key.
        recipient_private_key = "0x" + GENESIS_PRIV_KEY
        ...
        for i, v in enumerate(data_sizes):
            self.__test_ecies_upload_download(v, i + 1, recipient_private_key)

    def __test_ecies_upload_download(self, size, submission_index, recipient_private_key):
        self.log.info("ecies file size: %d", size)
        ...
        file_to_download = os.path.join(
            self.root_dir, "download_ecies_proof_%s" % root[:16]
        )
        ...
        file_to_download = os.path.join(
            self.root_dir, "download_ecies_noproof_%s" % root[:16]
        )
        self._download_file_use_cli(
            node_urls,
            None,
        ...
        self.log.info("ecies upload/download passed for size %d", size)
