...
impl Batcher {
    ...
    /// is settled on chain.
    pub fn new(version: u64, clients: Vec<ZgsClient>, web3_client: Web3Client) -> Self {
        Self {
    ...
    pub async fn exec(&self, option: &mut UploadOption) -> Result<H256> {
        // Build stream data
        ...
        let encoded = stream_data.encode().context("Failed to encode data")?;

        let data = DataInMemory::new(encoded).context("Failed to create data in memory")?;

        // Create uploader
        let uploader = Uploader::new(self.web3_client.clone(), self.clients.clone())
            .await
