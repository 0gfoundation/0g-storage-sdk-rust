...
pub struct GenerateArgs {
    ...
    #[arg(long, default_value = "false", help = "Whether to overwrite existing file")]
    pub overwrite: bool,

    #[arg(
        long,
        help = "File size in bytes (default \"[1M, 10M)\")"
    )]
...
pub async fn run_generate_file(args: &GenerateArgs) -> Result<()> {
    ...
    let data = File::open(args.file.clone()).unwrap();
    let tree = merkle_tree(Arc::new(data)).await?;
    log::info!("Succeeded to write file. root: {:?}, file: {:?}", tree.root(), file_name);
    Ok(())
}
