...
pub async fn merkle_tree(data: Arc<dyn IterableData>) -> Result<Tree> {
    ...
    log::debug!("data_size_origin: {}, num_segments_padded: {}, data_size_padded: {}", 
                data.size(), num_segments, data.padded_size());
    ...
    let segment_tasks = (0..num_segments).map(|i| {
        ...
        async move {
            ...
            log::debug!("Processed segment {}: offset = {}, size = {}, root = {:?}",
                        i, offset, segment_size, hash);
...
pub fn read_at(
    ...
    data.read(&mut buf, offset).map(|_| buf.clone()).or_else(|e| {
        if buf.is_empty() {
            Ok(vec![])
        } else {
...
pub async fn async_read_at(
    ...
    if offset < 0 || offset as u64 >= padded_size {
        return Err(anyhow!("Invalid offset: {}, padded size: {}", offset, padded_size));
    }
...
pub fn padded_segment_root(
    ...
    let num_segments_flow_padded = (num_chunks_flow_padded - 1) / DEFAULT_SEGMENT_MAX_CHUNKS as u64 + 1;
    ...
    if num_seg_chunks < DEFAULT_SEGMENT_MAX_CHUNKS as u64 {
        if segment_index < num_segments_flow_padded - 1 || num_chunks_flow_padded % DEFAULT_SEGMENT_MAX_CHUNKS as u64 == 0 {
    ...
    (segment_root(chunks, empty_chunks_padded), num_segments_flow_padded)
}
